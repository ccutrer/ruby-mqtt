...
describe "a client talking to a server" do # rubocop:disable RSpec/DescribeClass
  let(:error_log) { StringIO.new }
  let(:server) do
    MQTT::FakeServer.new.tap do |server|
  ...
  let(:client) { MQTT::Client.new(server.address, server.port) }
  ...
  context "when connecting and publishing a packet" do
  ...
  context "when connecting, subscribing to a topic and getting a packet" do
  ...
  context "when detecting then server not sending ping responses" do
    before do
