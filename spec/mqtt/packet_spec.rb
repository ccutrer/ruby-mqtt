...
describe MQTT::Packet do
  describe "when creating a new packet" do
    it "allows you to set the packet flags as a hash parameter" do
      packet = MQTT::Packet.new(flags: [true, false, true, false])
      expect(packet.flags).to eq([true, false, true, false])
    end

    it "has a custom inspect method" do
      packet = MQTT::Packet.new
      expect(packet.inspect).to eq("#<MQTT::Packet>")
    end

    it "has a type_id method to get the integer ID of the packet type" do
      packet = MQTT::Packet::Pingreq.new
      expect(packet.type_id).to eq(12)
    end
  end

  it "lets you change attributes using the update_attributes method" do
  ...
  describe "protected methods" do
    ...
    it "provides a encode_bytes method to get some bytes as Integers" do
      data = packet.send(:encode_bytes, 0x48, 0x65, 0x6c, 0x6c, "o".unpack1("C1"))
      expect(data).to eq("Hello")
    end

    it "provides a encode_bits method to encode an array of bits to a string" do
      data = packet.send(:encode_bits, [false, true, true, false, true, false, true, false])
      expect(data).to eq("V")
    end

    it "provides a add_short method to get a big-endian unsigned 16-bit integer" do
      data = packet.send(:encode_short, 1024)
      expect(data).to eq("\x04\x00")
      expect(data.encoding.to_s).to eq("ASCII-8BIT")
    end

    it "raises an error if too big argument for encode_short" do
      expect do
        packet.send(:encode_short, 0x10000)
      end.to raise_error(
        "Value too big for short"
      )
    end

    it "provides a add_string method to get a string preceeded by its length" do
      data = packet.send(:encode_string, "quack")
      expect(data).to eq("\x00\x05quack")
      expect(data.encoding.to_s).to eq("ASCII-8BIT")
    end

    it "provides a shift_short method to get a 16-bit unsigned integer" do
      buffer = +"\x22\x8Bblahblah"
      expect(packet.send(:shift_short, buffer)).to eq(8843)
      expect(buffer).to eq("blahblah")
    end

    it "provides a shift_byte method to get one byte as integers" do
      buffer = +"\x01blahblah"
      expect(packet.send(:shift_byte, buffer)).to eq(1)
      expect(buffer).to eq("blahblah")
    end

    it "provides a shift_bits method to get one byte as integers" do
      buffer = +"Yblahblah"
      expect(packet.send(:shift_bits, buffer)).to eq([true, false, false, true, true, false, true, false])
      expect(buffer).to eq("blahblah")
    end

    it "provides a shift_string method to get a string preceeded by its length" do
  ...
  describe "deprecated attributes" do
    it "stills have a message_id method that is that same as id" do
      packet = MQTT::Packet.new
  ...
  describe MQTT::Packet::Publish do
    describe "when creating a packet" do
      ...
      it "raises an exception the QoS is greater than 2" do
        expect do
          MQTT::Packet::Publish.new(qos: 3)
        end.to raise_error(
          "Invalid QoS value: 3"
        )
      end

      it "raises an exception the QoS is less than 0" do
        expect do
          MQTT::Packet::Publish.new(qos: -1)
        end.to raise_error(
          "Invalid QoS value: -1"
        )
      end
    ...
    describe "when setting attributes on a packet" do
      ...
      it "lets you change the dup flag of a packet" do
        packet.duplicate = true
        expect(packet.duplicate).to be_truthy
      end

      it "lets you change the dup flag of a packet using an integer" do
        packet.duplicate = 1
        expect(packet.duplicate).to be_truthy
      end

      it "lets you change the QoS value of a packet" do
        packet.qos = 1
        expect(packet.qos).to eq(1)
      end

      it "lets you change the retain flag of a packet" do
        packet.retain = true
        expect(packet.retain).to be_truthy
      end

      it "lets you change the retain flag of a packet using an integer" do
        packet.retain = 1
        expect(packet.retain).to be_truthy
      end
    end
    ...
    describe "when serialising a packet" do
      it "outputs the correct bytes for a packet with default QoS and no flags" do
        packet = MQTT::Packet::Publish.new(topic: "test", payload: "hello world")
        expect(packet.to_s).to eq("\x30\x11\x00\x04testhello world")
      end

      it "outputs the correct bytes for a packet with QoS 1 and no flags" do
      ...
      it "outputs the correct bytes for a packet with QoS 2 and retain flag set" do
      ...
      it "outputs the correct bytes for a packet with QoS 2 and dup flag set" do
      ...
      it "raises an exception when there is an empty topic name" do
        expect do
    ...
    describe "when serialising an oversized packet" do
      it "raises an exception when body is bigger than 256MB" do
        expect do
          packet = MQTT::Packet::Publish.new(topic: "test", payload: "x" * 268_435_455)
          packet.to_s
        end.to raise_error(
    ...
    describe "when parsing a packet with QoS 0" do
      ...
      it "correctly creates the right type of packet object" do
        expect(packet.class).to eq(MQTT::Packet::Publish)
      end

      it "sets the QoS level correctly" do
        expect(packet.qos).to eq(0)
      end

      it "sets the RETAIN flag correctly" do
        expect(packet.retain).to be_falsey
      end

      it "sets the DUP flag correctly" do
        expect(packet.duplicate).to be_falsey
      end

      it "sets the topic name correctly" do
    ...
    describe "when parsing a packet with a QoS value of 3" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x36\x12\x00\x03a/b\x00\x05hello world")
        end.to raise_error(
    ...
    describe "when parsing a packet with QoS value of 0 and DUP set" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x38\x10\x00\x03a/bhello world")
        end.to raise_error(
          MQTT::ProtocolException,
    ...
    describe "when parsing a packet with a body of 314 bytes" do
      ...
      it "parses the packet type correctly" do
        expect(packet.class).to eq(MQTT::Packet::Publish)
      end

      it "gets the topic name correctly" do
        expect(packet.topic).to eq("topic")
      end

      it "gets the body length correctly" do
    ...
    describe "when parsing a packet with a body of 16kbytes" do
      let(:packet) do
        ...
        # 0x80 = (0 * 128)
        # 0x01 = (1 * 16384)
        MQTT::Packet.parse("\x30\x87\x80\x01\x00\x05topic#{"x" * 16_384}")
      end
      ...
      it "gets the body length correctly" do
        expect(packet.payload.bytesize).to eq(16_384)
      end
    end
    ...
    describe "processing a packet containing UTF-8 character" do
      let(:packet) do
        MQTT::Packet::Publish.new(
          topic: (+"Test \xE2\x91\xA0").force_encoding("UTF-8"),
          payload: (+"Snowman: \xE2\x98\x83").force_encoding("UTF-8")
        )
      end

      it "has the correct topic byte length" do
        expect(packet.topic.bytesize).to eq(8)
      end

      it "has the correct topic string length" do
        expect(packet.topic.length).to eq(6)
      end

      it "has the correct payload byte length" do
        expect(packet.payload.bytesize).to eq(12)
      end

      it "has the correct payload string length" do
        expect(packet.payload.length).to eq(10)
      end

      it "encodes to MQTT packet correctly" do
        expect(packet.to_s).to eq((+"\x30\x16\x00\x08Test \xE2\x91\xA0Snowman: \xE2\x98\x83").force_encoding("BINARY"))
      end

      it "parses the serialised packet" do
        packet2 = MQTT::Packet.parse(packet.to_s)
        expect(packet2.topic).to eq((+"Test \xE2\x91\xA0").force_encoding("UTF-8"))
        expect(packet2.payload).to eq((+"Snowman: \xE2\x98\x83").force_encoding("BINARY"))
      end
    end
    ...
    describe "reading a packet from a socket" do
      ...
      it "sets the body length is read correctly" do
    ...
    describe "when calling the inspect method" do
      it "outputs the payload, if it is less than 16 bytes" do
      ...
      it "outputs the length of the payload, if it is more than 16 bytes" do
      ...
      it "onlies output the length of a binary payload" do
  ...
  describe MQTT::Packet::Connect do
    describe "when serialising a packet" do
      ...
      context "with protocol version 3.1.0" do
        ...
        it "raises an exception when the client identifier is too long" do
          expect do
      ...
      context "with protocol version 3.1.1" do
        it "allows no client identifier" do
          packet = MQTT::Packet::Connect.new(
            version: "3.1.1",
            client_id: "",
            clean_session: true
        ...
        it "allows a 32 character client identifier" do
          client_id = "0EB8D2FE7C254715B4467C5B2ECAD100"
          packet = MQTT::Packet::Connect.new(
            version: "3.1.1",
            client_id:,
      ...
      it "raises an exception if the keep alive value is less than 0" do
        expect do
      ...
      it "outputs the correct bytes for a packet with a Will" do
        packet = MQTT::Packet::Connect.new(
          client_id: "myclient",
          clean_session: true,
          will_qos: 1,
      ...
      it "outputs the correct bytes for a packet with a username and password" do
        packet = MQTT::Packet::Connect.new(
          client_id: "myclient",
          username: "username",
          password: "password"
      ...
      it "outputs the correct bytes for a packet with everything" do
        packet = MQTT::Packet::Connect.new(
      ...
      context "with an invalid protocol version number" do
        it "raises a protocol exception" do
          expect do
            MQTT::Packet::Connect.new(version: "x.x.x", client_id: "myclient")
          end.to raise_error(
            ArgumentError,
    ...
    describe "when parsing a simple 3.1.0 Connect packet" do
      ...
      it "sets the fixed header flags of the packet correctly" do
        expect(packet.flags).to eq([false, false, false, false])
      end

      it "sets the Protocol Name of the packet correctly" do
        expect(packet.protocol_name).to eq("MQIsdp")
        expect(packet.protocol_name.encoding.to_s).to eq("UTF-8")
      end

      it "sets the Protocol Level of the packet correctly" do
        expect(packet.protocol_level).to eq(3)
      end

      it "sets the Protocol version of the packet correctly" do
      ...
      it "sets not have the clean session flag set" do
        expect(packet.clean_session).to be_falsey
      end

      it "sets the the username field of the packet to nil" do
        expect(packet.username).to be_nil
      end

      it "sets the the password field of the packet to nil" do
    ...
    describe "when parsing a Connect packet with a Will and Testament" do
      ...
      it "sets the QoS of the Will should be 1" do
      ...
      it "sets the Will topic of the packet correctly" do
        expect(packet.will_topic).to eq("topic")
        expect(packet.will_topic.encoding.to_s).to eq("UTF-8")
      end

      it "sets the Will payload of the packet correctly" do
    ...
    describe "when parsing a Connect packet with a username and password" do
      ...
      it "sets the username of the packet correctly" do
        expect(packet.username).to eq("username")
        expect(packet.username.encoding.to_s).to eq("UTF-8")
      end

      it "sets the password of the packet correctly" do
    ...
    describe "when parsing a Connect packet with every option set" do
      ...
      it "sets the Keep Alive Timer of the packet correctly" do
        expect(packet.keep_alive).to eq(65_535)
      end
      ...
      it "sets the Will QoS of the packet correctly" do
        expect(packet.will_qos).to eq(2)
      end

      it "sets the Will retain flag of the packet correctly" do
    ...
    describe "when calling the inspect method" do
      it "outputs correct string for the default options" do
        packet = MQTT::Packet::Connect.new
        expect(packet.inspect).to eq("#<MQTT::Packet::Connect: keep_alive=15, clean, client_id=''>")
      end

      it "outputs correct string when parameters are given" do
        packet = MQTT::Packet::Connect.new(
          keep_alive: 10,
          client_id: "c123",
          clean_session: false,
    ...
    describe "deprecated attributes" do
      it "stills have a protocol_version method that is that same as protocol_level" do
        packet = MQTT::Packet::Connect.new
  ...
  describe MQTT::Packet::Connack do
    describe "when setting attributes on a packet" do
      ...
      it "lets you change the session present flag of a packet" do
        packet.session_present = true
        expect(packet.session_present).to be_truthy
      end

      it "lets you change the session present flag of a packet using an integer" do
        packet.session_present = 1
        expect(packet.session_present).to be_truthy
      end

      it "lets you change the return code of a packet" do
    ...
    describe "when serialising a packet" do
      it "outputs the correct bytes for a sucessful connection acknowledgement packet without Session Present set" do
        packet = MQTT::Packet::Connack.new(return_code: 0x00, session_present: false)
        expect(packet.to_s).to eq("\x20\x02\x00\x00")
      end

      it "outputs the correct bytes for a sucessful connection acknowledgement packet with Session Present set" do
    ...
    describe "when parsing a successful Connection Accepted packet" do
      ...
      it "sets the Session Pression flag of the packet correctly" do
        expect(packet.session_present).to be(false)
      end
    ...
    describe "when parsing a successful Connection Accepted packet with Session Present set" do
      ...
      it "sets the Session Pression flag of the packet correctly" do
        expect(packet.session_present).to be(true)
      end
    ...
    describe "when parsing packet with invalid Connack flags set" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x20\x02\xff\x05")
        end.to raise_error(
          MQTT::ProtocolException,
    ...
    describe "when parsing packet with extra bytes on the end" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x20\x03\x00\x00\x00")
        end.to raise_error(
          MQTT::ProtocolException,
    ...
    describe "when calling the inspect method" do
      it "outputs the right string when the return code is 0" do
        packet = MQTT::Packet::Connack.new(return_code: 0x00)
        expect(packet.inspect).to eq("#<MQTT::Packet::Connack: 0x00>")
      end

      it "outputs the right string when the return code is 0x0F" do
  ...
  describe MQTT::Packet::Puback do
    ...
    describe "when parsing packet with extra bytes on the end" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x40\x03\x12\x34\x00")
        end.to raise_error(
          MQTT::ProtocolException,
    ...
    it "outputs the right string when calling inspect" do
      packet = MQTT::Packet::Puback.new(id: 0x1234)
  ...
  describe MQTT::Packet::Pubrec do
    ...
    describe "when parsing packet with extra bytes on the end" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x50\x03\x12\x34\x00")
        end.to raise_error(
          MQTT::ProtocolException,
  ...
  describe MQTT::Packet::Pubrel do
    ...
    describe "when parsing packet with extra bytes on the end" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\x62\x03\x12\x34\x00")
        end.to raise_error(
          MQTT::ProtocolException,
  ...
  describe MQTT::Packet::Subscribe do
    describe "setting the packet's topics" do
      ...
      it "is able to set the topics from a String 'a/b'" do
        packet.topics = "a/b"
        expect(packet.topics).to eq([["a/b", 0]])
      end

      it "is able to set the multiple topics from an array ['a/b', 'b/c']" do
        packet.topics = ["a/b", "b/c"]
        expect(packet.topics).to eq([["a/b", 0], ["b/c", 0]])
      end

      it "is able to set the topics from a Hash {'a/b' => 0, 'b/c' => 1}" do
        packet.topics = { "a/b" => 0, "b/c" => 1 }
        expect(packet.topics).to eq([["a/b", 0], ["b/c", 1]])
      end

      it "is able to set the topics from a single level array ['a/b', 0]" do
        packet.topics = ["a/b", 0]
        expect(packet.topics).to eq([["a/b", 0]])
      end

      it "is able to set the topics from a two level array [['a/b' => 0], ['b/c' => 1]]" do
        packet.topics = [["a/b", 0], ["b/c", 1]]
        expect(packet.topics).to eq([["a/b", 0], ["b/c", 1]])
      end

      it "raises an exception when setting topic with a non-string" do
        expect do
          packet.topics = 56
        end.to raise_error(
    ...
    describe "when serialising a packet" do
      it "outputs the correct bytes for a packet with a single topic" do
        packet = MQTT::Packet::Subscribe.new(id: 1, topics: "a/b")
        expect(packet.to_s).to eq("\x82\x08\x00\x01\x00\x03a/b\x00")
      end

      it "outputs the correct bytes for a packet with multiple topics" do
      ...
      it "raises an exception when no topics are given" do
        expect do
    ...
    describe "when parsing a packet with a single topic" do
      ...
      it "sets the Message ID correctly" do
  ...
  describe MQTT::Packet::Suback do
    describe "when serialising a packet" do
      it "outputs the correct bytes for an acknowledgement to a single topic" do
        packet = MQTT::Packet::Suback.new(id: 5, return_codes: 0)
        expect(packet.to_s).to eq("\x90\x03\x00\x05\x00")
      end

      it "outputs the correct bytes for an acknowledgement to a two topics" do
        packet = MQTT::Packet::Suback.new(id: 6, return_codes: [0, 1])
        expect(packet.to_s).to eq("\x90\x04\x00\x06\x00\x01")
      end

      it "raises an exception when no granted QoSs are given" do
        expect do
      ...
      it "raises an exception if the granted QoS is not an integer" do
        expect do
    ...
    describe "when parsing a packet with a single QoS value of 0" do
      ...
      it "sets the Granted QoS of the packet correctly" do
    ...
    describe "when calling the inspect method" do
      it "outputs correct string for a single granted qos" do
    ...
    describe "deprecated attributes" do
      it "stills have a granted_qos method that is that same as return_codes" do
  ...
  describe MQTT::Packet::Unsubscribe do
    describe "when serialising a packet" do
      it "outputs the correct bytes for a packet with single topic" do
    ...
    describe "when calling the inspect method" do
      ...
      it "outputs correct string for multiple topics" do
        packet = MQTT::Packet::Unsubscribe.new(id: 42, topics: %w[a b c])
  ...
  describe MQTT::Packet::Unsuback do
    ...
    describe "when parsing packet with extra bytes on the end" do
      it "raises an exception" do
        expect do
          MQTT::Packet.parse("\xB0\x03\x12\x34\x00")
        end.to raise_error(
          MQTT::ProtocolException,
  ...
  describe MQTT::Packet::Pingreq do
    ...
    describe "when parsing a packet" do
      ...
      it "raises an exception if the packet has a payload" do
        expect do
  ...
  describe "Serialising an invalid packet that has no type" do
